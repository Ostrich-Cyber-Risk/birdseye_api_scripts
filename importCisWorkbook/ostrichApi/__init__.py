...
def _handle_response(response: requests.Response) -> Any:
    ...
    body = response.json()
    # Reads nest their payload under "response"; the save endpoints reply with a bare
    # {"message": ...} and no wrapper at all.
...
class OstrichApi:
    ...
    def get_assessment_content(self, business_unit_id: str, assessment_id: str) -> dict:
        """Return the assessment's content spec: its "descriptor", "questions" and "nodes".

        The endpoint nests the spec under a "content" key alongside the business unit and
        assessment ids, which the caller already knows. Unwrap it.
        """
        url = f'{self._base_url}/v1/businessUnits/{business_unit_id}/assessments/{assessment_id}/content'
        return _handle_response(self.__call(requests.get, url)).get('content') or {}
